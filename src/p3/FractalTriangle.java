...
public class FractalTriangle
{
    ...
    public FractalTriangle( int depth, ATriangle tri, int height ) 
    {
        ...
        if( depth > 1 && height > minHeight )
        {
            if ( depth <= maxDepth )
            {
                Point[] points = _tri.getPoints();
                arrayFT = new FractalTriangle[3];
                arrayFT[0] = new FractalTriangle( this.makeTriangle( 
                    points[0], points[1] ), this, depth - 1, 
                                                 this.makeTriangle( points[0], 
                                                     points[1] ).getHeight() ); 
                arrayFT[1] = new FractalTriangle( this.makeTriangle( points[1],
                    points[2] ), this, depth - 1, this.makeTriangle( points[0], 
                                                     points[1] ).getHeight() );
                arrayFT[2] = new FractalTriangle( this.makeTriangle( points[2],
                                                 points[0] ), this, depth - 1, 
                                                 this.makeTriangle( points[0],
                                                 points[1] ).getHeight() );
            }
        }
                             ...
                             int depth , int height )
    {
        ...
        _depth = depth;
        _parent = parent;
        //_tri.setColor( colors[ oDepth - depth ]);
        triangleColor = colors[ oDepth - depth ];
        ///////////////////////////////////////////////////////////
        ...
        if( depth > 1 && height > minHeight ) 
        {
            if ( depth <= maxDepth ) 
            {
                ...
                arrayFT[0] = new FractalTriangle( this.makeTriangle( 
                              points[0], points[1] ),
                                this, depth - 1, this.makeTriangle( 
                                     points[0], points[1] ).getHeight() );
                arrayFT[1] = new FractalTriangle( this.makeTriangle( 
                             points[1], points[2] ),
                                          this, depth - 1, this.makeTriangle( 
                                              points[0], points[1] 
                                          ).getHeight() );
                arrayFT[2] = new FractalTriangle( 
                                     this.makeTriangle( points[2], points[0] ),
                    this, depth - 1, this.makeTriangle( 
                        points[0], points[1] ).getHeight() );
            }
        }
    ...
    public void display ( Graphics2D context )
    {
        ...
        if( fillTriangles )
        {
            _tri.setColor( triangleColor );
            
        } else 
        {
            Color color = new Color( 1, 0, 0, 0 ); //clear
            _tri.setColor( color );
        }
        
        if( outside )
        {
            _tri.setFrameColor( triangleColor );
        }
        ...
        if( arrayFT != null )
        {
            for( int i = 0; i < arrayFT.length; i++ )
            {
                arrayFT[i].display( context );
            }
        }
    }
    ////////////////////////////////////////////////////////////////////////
